...
// API Base URL
const API_URL = 'https://medinet360-api.onrender.com';
// --- Initialization ---
...
// --- Export Logic (Now using Backend API) ---
async function exportToCSV() {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/api/reports/patients/csv?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.status === 403) {
            showToast('Esta función requiere el plan Clinic Plus', 'error');
            return;
        }
        if (!response.ok) {
        ...
        showToast('Error al generar el reporte CSV', 'error');
    }
}
async function exportListToPDF() {
    ...
    try {
        ...
        if (startDateInput.value) params.append('startDate', startDateInput.value);
        if (endDateInput.value) params.append('endDate', endDateInput.value);
        const response = await fetch(`${API_URL}/api/reports/patients/pdf?${params.toString()}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.status === 403) {
            showToast('Esta función requiere el plan Clinic Plus', 'error');
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `reporte_pacientes_${new Date().toISOString().split('T')[0]}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        window.URL.revokeObjectURL(url);
...
async function generateClinicalPDF() {
    ...
    try {
        const response = await fetch(`${API_URL}/api/reports/clinical/${selectedPatient._id}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.status === 403) {
            showToast('Esta función requiere el plan Clinic Plus', 'error');
        ...
        if (!response.ok) {
            throw new Error('Error al generar la ficha clínica');
        }
        const blob = await response.blob();
        const url = window.URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = `Ficha_${selectedPatient.name}_${selectedPatient.lastName}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        window.URL.revokeObjectURL(url);
        showToast('Ficha clínica generada exitosamente', 'success');
    } catch (error) {
        console.error('Error generating clinical PDF:', error);
        showToast('Error al generar la ficha clínica', 'error');
    }
}
async function generateHistoryPDF() {
    ...
    try {
        const response = await fetch(`${API_URL}/api/reports/history/${selectedHistoryPatient._id}`, {
            headers: { 'Authorization': `Bearer ${token}` }
        });
        if (response.status === 403) {
            showToast('Esta función requiere el plan Clinic Plus', 'error');
        ...
        const a = document.createElement('a');
        a.href = url;
        a.download = `Historial_Citas_${selectedHistoryPatient.name}.pdf`;
        document.body.appendChild(a);
        a.click();
        a.remove();
        window.URL.revokeObjectURL(url);
        showToast('Historial de citas generado exitosamente', 'success');
    } catch (error) {
        console.error('Error generating history PDF:', error);
        showToast('Error al generar el historial de citas', 'error');
    }
}